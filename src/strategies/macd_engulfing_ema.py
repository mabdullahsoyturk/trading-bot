...
class MacdEngulfingStrategy():
    def __init__(self, ohlcvs, ohlcv_data):
        self.ohlcvs = ohlcvs
        self.ohlcv_data = ohlcv_data

    def execute(self):
        long_position = self.macd_engulf_long()
        short_position = self.macd_engulf_short()
    ...
    def macd_engulf_long(self):
        ...
        if first_candle.close_price < first_candle.open_price and \
            ...
            print(f'\n[LONG][{open_time}] Opened at: {entry_price}, Stop Loss: {stop_loss}, Take Profit: {take_profit}, EMA: {ema[-2]} MACD: {MACD[-2]}')
            position = Position("buy", open_time, entry_price, stop_loss, take_profit, ema[-2], atr[-2])
            
            return position

    def macd_engulf_short(self):
        num_stops = 0
        num_take_profits = 0
        num_positions = 0
        ...
        if first_candle.close_price > first_candle.open_price and \
            ...
            print(f'\n[SHORT][{open_time}] Opened at: {entry_price}, Stop Loss: {stop_loss}, Take Profit: {take_profit}, EMA: {ema[-2]} MACD: {macd[-2]}')
